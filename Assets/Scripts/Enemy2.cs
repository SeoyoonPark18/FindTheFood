...
public class Enemy2 : MonoBehaviour
{
    ...
    public float attackDist = 30; //공격 거리
    ...
    void Update()
    {
        ...
        if (dist < attackDist) //지정한 거리보다 가깝다면
        {
            ...
            {
                int enemyCount = GameObject.FindGameObjectsWithTag("ZOMBIE").Length; //좀비(자신)가 몇마리 남아있는지
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.collider.tag == "PBULLET") //만약 주인공 총알에 맞는다면
        {
            ...
            {
                Enemy1.killedEnemy++; //적 제거 횟수 증가
                print("적제거: " + Enemy1.killedEnemy); //콘솔창에 표시
